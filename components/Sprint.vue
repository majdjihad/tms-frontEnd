...
const {deleteSprint, createIssue, moveIssue, moveMultipleIssue, deleteMultipleIssues, editMultipleIssue} = useBacklog()
const inputFocus = ref(null)
const openCreateIssue = () => creating.value = true;
...
const handleDeleteSprint = () => {
  ...
  const {submit} = useSubmit(async () => {
    return await deleteSprint(projectsStore?.project?.project_identify, props?.sprint?.id, {'issue_sprint_id': null})
  }, {
    onSuccess: async (response) => {
    },
    onError: async (error) => {
...
const handleOutsideClick = (e) => {
  if (document.getElementById("createIssueTopButton").contains(e.target)) {
    return;
  }

  if (!document.getElementById('createIssueWrapper').contains(e.target)) {
    closeInput();
      ...
      <div class="card-header py-4">
        <div class="card-title d-flex flex-column">
          <div class="d-flex align-items-center position-relative mb-2">
            <div class="d-flex justify-content-between align-items-center cursor-pointer me-3" @click="toggleArrow"
                 data-bs-toggle="collapse" :data-bs-target="`#collapseSprint${sprint?.id}`" aria-expanded="true"
                 :aria-controls="`collapseSprint${sprint?.id}`">
              <h2 class="me-3">{{ sprint?.name }}</h2>
              <icon :name="arrowIcon" color="black" class="cursor-pointer fw-bold ms-2 p-1"
                    size="25"/>
            </div>

            <span v-if="sprint?.issues.length > 0" class="fs-6 fw-normal badge badge-light-success text-success px-3">{{
          ...
          <p v-if="sprint?.goal" class="fs-6 m-0 mt-2 text-gray-600 fw-normal truncate" style="max-width: 700px;">
            {{ sprint?.goal }}</p>
        </div>
        <div class="card-toolbar">
          <div class="d-flex justify-content-end gap-4" data-kt-customer-table-toolbar="base">
            <button @click="scrollToCreate" id="createIssueTopButton" type="button"
                    class="btn btn-light-primary btn-sm">
              <i class="ki-outline ki-plus fs-2"></i>Create Issue
            </button>
            <div class="position-relative symbol">
              <div class="btn btn-sm btn-icon btn-bg-light" @click="toggleSprintMenu" aria-haspopup="true"
                   aria-controls="overlay_tmenu">
                <i class="ki-solid ki-dots-horizontal fs-2x"></i>
              </div>
                <TieredMenu ref="sprintActionMenu" id="overlay_tmenu" :model="sprintActionItems" popup/>
            </div>
          </div>
        </div>
      </div>
      <div class="card-body p-0 collapse show" :id="`collapseSprint${sprint?.id}`">
        <div>
          <div v-if="selectedIssue.length > 0"
            ...
            <div class="d-flex justify-content-between align-items-center gap-2">
              <div>
                <div class="btn btn-sm btn-icon btn-bg-light" v-click-out-side="closeIssueActionMenu"
                     @click="toggleIssueActionVisible">
                  <i class="ki-solid ki-dots-horizontal fs-2x"></i>
                </div>
                <Transition name="menu">
                  <div class="position-relative" v-show="issuesActionMenu">
                    <ul class="position-absolute m-0 mt-2 p-0 top-100 left-0 bg-white list-style-none z-index-3 w-90px">
                      <li class="p-3 hover-bg-light cursor-pointer fs-8 fw-normal text-start" @click="selectAllBtn">
                        Select all
                      </li>
                      <li class="p-3 hover-bg-light cursor-pointer fs-8 fw-normal text-start" @click="clearAllBtn">Clear
                        all
                      </li>
                    </ul>
                  </div>
            ...
            <Draggable v-model="sprint.issues"
            ...
            >
              <template #item="{element: issue}">
                <IssueComponentMainIssue :data-draggable="issue" v-model:input="selectedIssue" :issue="issue"/>
              </template>
            </Draggable>
          <div v-if="inProgress || createProgress">
            <IssueComponentMainIssue :data-draggable="issue" :issue="data" :progress="true"/>
          </div>
          <div v-show="!sprint?.issues?.length > 0 && (!inProgress && !createProgress)"
          ...
          <div @click.stop id="createIssueWrapper" ref="inputFocus" @click="openCreateIssue"
            ...
            <div v-if="creating" class="d-flex align-items-center">
              <div class="position-relative">
                <div class="py-1 px-3 rounded-1 fw-bold fs-8 text-uppercase text-nowrap">
                  <div v-click-out-side="closeTypeIssueMenu" @click="typeIssueMenuToggle">
                    <img v-if="typeIssue === 'task'" src="~/assets/media/issue/type/task.svg"/>
                    <img v-else-if="typeIssue === 'story'" src="~/assets/media/issue/type/story.svg"/>
                    <img v-else-if="typeIssue === 'bug'" src="~/assets/media/issue/type/bug.svg"/>
                ...
                <Transition name="statusesMenu">
                  <div v-if="openTypeIssueMenu"
                    ...
                    <div v-if="typeIssue !== 'task'" class="p-1 hover-bg-light">
                      <div class=" fs-7 d-flex justify-content-start"
                           style="max-width: 150px"
                           @click="changeTypeIssue('task')">
                        <img class="py-1 px-1 rounded-1 me-1" src="~/assets/media/issue/type/task.svg"/>
                        <span class="fs-6">task</span>
                      </div>
                    </div>
                    <div v-if="typeIssue !== 'story'" class="p-1 hover-bg-light">
                      <div class=" fs-7 d-flex justify-content-start"
                           style="max-width: 150px"
                           @click="changeTypeIssue('story')">
                        <img class="py-1 px-1 rounded-1 me-1" src="~/assets/media/issue/type/story.svg"/>
                        <span class="fs-6">story</span>
                      </div>
                    </div>
                    <div v-if="typeIssue !== 'bug'" class="p-1 hover-bg-light">
                      <div class=" fs-7 d-flex justify-content-start"
                           style="max-width: 150px"
                           @click="changeTypeIssue('bug')">
                        <img class="py-1 px-1 rounded-1 me-1" src="~/assets/media/issue/type/bug.svg"/>
              ...
              <FormInput class="w-90 m-0 " @keyup.enter="createIssueToSprint" type="text" autocomplete="off"
                         labelText="Issue Title:" name="title" placeholder="Issue Title"
                         v-model:input="createIssueInput" :formDataError="errorMsg.errorTitle"
                         :focus="true"/>
            </div>
            <span @click="scrollToCreate" id="createIssueTopButton" class="d-flex align-items-center py-2"><i
