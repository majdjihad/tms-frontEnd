...
const props = defineProps(["issue", "progress", "input", "sprint", "canEdite"]);
...
const changeIssue = async (actionId) => {
  ...
  Object.keys(actionId)[0] === "status_id"
    ...
    : false;
  try {
    await editIssue(route.params.id, props?.issue?.id, actionId);
    await backlogStore?.getBacklogProject(route.params.id);
  } catch (error) {
              ...
              items: [
                {
                  label: "Top",
                  command: async () => {
                    await moveIssue(route.params.id, props?.issue?.id, {
                      sprint_id: null,
                      position: 1,
                      order: 0,
                    });
...
const changePriority = async (priorityObj) => {
  ...
  try {
    await editPriority(route.params.id, props?.issue?.id, priorityObj);
    await backlogStore?.getBacklogProject(route.params.id);
  } catch (error) {
    showToast(error?.data?.message);
  }
...
<template>
  <div>
    <div
      v-if="!progress"
      :class="{
        'issue-border': model?.includes(issue?.id),
        'cursor-grab': canEdite,
        'issue-for-check': canEdite,
      }"
      class="issue-container row justify-content-between px-8 m-0 border-bottom position-relative"
    >
      <div
        class="row col-8 col-md-6 justify-content-around align-items-center form-check-sm"
      >
        <div
        ...
        <div
          class="issue-title col-11 h-100 d-flex align-items-center py-3 cursor-pointer"
      ...
      >
        <div class="position-relative col-auto">
          <div v-if="canEdite || issue?.team_member?.user?.id === user?.id">
            <div
...
.issue-for-check :hover .form-check-input {
  display: inline;
}
