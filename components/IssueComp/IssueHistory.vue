...
<template>
  <div class="comment-content px-3 py-6 rounded-1 border-bottom">
    <div class="d-flex justify-content-between align-center">
      ...
      <div class="d-flex align-items-center overflow-hidden">
        <div class="d-flex justify-content-between align-center">
          ...
          <span class="text-gray-600 ms-1 fs-7">{{
    ...
    <div
      v-if="history?.type === 'type'"
      class="fs-6 d-flex align-items-center mt-3"
    >
      <div>
        <img
          v-if="history?.old_data === 'task'"
          alt="task"
          src="~/assets/media/issue/type/task.svg"
        />
        <img
          v-else-if="history?.old_data === 'story'"
          alt="story"
          src="~/assets/media/issue/type/story.svg"
        />
        <img
          v-else-if="history?.old_data === 'bug'"
          alt="bug"
          src="~/assets/media/issue/type/bug.svg"
        />
        <span class="ms-1 fw-normal">{{ history?.old_data }}</span>
      </div>
      <i class="fa-solid fa-arrow-right fs-4 mx-4"></i>
      <div>
        <img
          v-if="history?.new_data === 'task'"
          alt="task"
          src="~/assets/media/issue/type/task.svg"
        />
        <img
          v-else-if="history?.new_data === 'story'"
          alt="story"
          src="~/assets/media/issue/type/story.svg"
        />
        <img
          v-else-if="history?.new_data === 'bug'"
          alt="bug"
          src="~/assets/media/issue/type/bug.svg"
        />
        <span class="ms-1 fw-normal">{{ history?.new_data }}</span>
      </div>
    </div>
    <div
