...
const selectAllBtn = () => {
  selectedIssue.value = [];
  for (const issue in backlogStore?.backlogProject) {
    selectedIssue.value.push(backlogStore?.backlogProject[issue]?.id);
  }
};
...
watch(statusSelected, () => {
  statusSelected.value
    ? handleMultipleEditIssue("status_id", statusSelected?.value?.id)
    : false;
});
...
watch(assigneeSelected, () => {
  assigneeSelected.value
    ? handleMultipleEditIssue("assign_to", assigneeSelected?.value?.id)
    : false;
});

const handleMultipleEditIssue = (keyParam, valueParam) => {
  Swal.fire({
    html: `Are you sure you want to change the ${
      keyParam == "status_id"
        ? "status"
        : keyParam == "assign_to"
        ? "assign"
        : ""
    } for <strong><div class="truncate lh-lg" >${
      selectedIssue?.value?.length
    } issues</div></strong>`,
    icon: "info",
    buttonsStyling: false,
    showCancelButton: true,
        ...
        <div class="card-title d-flex flex-column">
          <div class="d-flex align-items-center position-relative">
            <div
              ...
              @click="
                arrowIcon === 'ep:arrow-up-bold'
                  ? (arrowIcon = 'ep:arrow-down-bold')
                  : (arrowIcon = 'ep:arrow-up-bold')
              "
              data-bs-toggle="collapse"
              data-bs-target="#collapseBacklog"
              aria-expanded="true"
            ...
            >
              ...
              <icon
                :name="arrowIcon"
                color="black"
                class="cursor-pointer ms-2 p-1"
                size="25"
              />
            </div>
            <span
              v-if="backlogStore?.backlogProject?.length > 0"
              class="fs-6 fw-normal badge badge-light-success text-success px-3"
              >{{ backlogStore?.backlogProject?.length }} issues</span
            >
          </div>
        </div>
        <div class="card-toolbar">
          <div class="d-flex justify-content-end gap-4">
            ...
            <button
              ...
              id="createBacklogIssueTopButton"
              type="button"
        ...
        <div>
          ...
          >
            <div class="d-flex flex-column text-center">
              <h4 class="fw-semibold p-0 m-0 text-muted">
