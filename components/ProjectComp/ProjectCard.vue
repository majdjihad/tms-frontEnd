    ...
    <div class="card shadow-sm">
      <div class="card-header border-0 px-7 pt-7">
        ...
        <div class="card-toolbar">
          <button
            :disabled="inProgress"
            class="project-fav border-0 badge fw-bold me-auto px-4 py-3 badge-light-danger cursor-pointer"
            @click="
              favorite(project?.project_identify);
      ...
      <div class="card-body p-7">
        <NuxtLink
          ...
          class="project-name fs-3 fw-bold text-dark"
        >
          {{ project?.name }}
          <Icon name="ci:external-link" size="20" />
        </NuxtLink>
        <p class="text-gray-400 fw-semibold fs-5 mt-1 mb-7 truncate">
          {{ project?.description }}
        </p>
        ...
        <div class="separator d-flex flex-center mb-8">
          <span class="text-uppercase bg-body fs-7 fw-semibold text-muted px-3"
            ><Icon name="ri:team-line" size="18"
          /></span>
        </div>
        <div
          v-if="project?.team_members?.length > 0"
          class="symbol-group symbol-hover flex-nowrap me-5"
        >
          <div v-for="(member, index) in project?.team_members" :key="index">
            <NuxtLink
              :to="`/profile/${member?.user?.identify_number}`"
              v-if="index <= 7 && member.invite_status === 'accept'"
            ...
            >
              ...
              <span
                v-else
                class="symbol-label text-inverse-warning text-uppercase"
        ...
        <div
          v-else
          class="alert bg-light-danger d-flex align-items-center w-100"
        >
          <Icon
            name="fluent:people-error-16-regular"
            size="24"
            class="text-danger me-4"
          />

          <div class="w-100 d-flex align-items-center">
            <h5 class="fw-semibold p-0 m-0 text-muted">No Team Found.</h5>
          </div>
        </div>
      </div>
    </div>
