...
const {submit, inProgress} = useSubmit(async () => {
  return await deleteIssue(projectsStore?.project?.project_identify, props?.subIssue?.id)
}, {
  onSuccess: async (response) => {
    await backlogStore?.getIssueInfo(projectsStore?.project?.project_identify, props?.subIssue?.parent_id);
  },
  onError: async (error) => {
...
const getInfoSubIssue = async () => {
  ...
  await backlogStore?.getIssueInfo(projectsStore?.project?.project_identify, props?.subIssue?.parent_id);
  await backlogStore?.getIssueComments(projectsStore?.project?.project_identify, props?.subIssue?.id);
}
...
const changesubIssue = async (actionId) => {
  ...
  try {
    await editIssue(projectsStore?.project?.project_identify, props?.subIssue?.id, actionId);
    await backlogStore?.getIssueInfo(projectsStore?.project?.project_identify, props?.subIssue?.parent_id);
  } catch (error) {
...
const changePriority = async (priorityObj) => {
  ...
  try {
    await editPriority(projectsStore?.project?.project_identify, props?.subIssue?.id, priorityObj);
...
<template>
  <div
    ...
    <div data-bs-toggle="modal" data-bs-target="#kt_modal_subIssue_info" @click="getInfoSubIssue">
      <img v-if="subIssue?.type === 'task'" src="../../assets/media/issue/type/task.svg" class="me-2" alt="task">
      <img v-else-if="subIssue?.type === 'story'" src="../../assets/media/issue/type/story.svg" class="me-2"
           alt="story">
      <img v-else-if="subIssue?.type === 'bug'" src="../../assets/media/issue/type/bug.svg" class="me-2" alt="bug">
      <span class="text-gray-600 me-2 truncate text-nowrap fs-8">{{ subIssue?.key }}</span>
      <span class="text-gray-800 me-2 truncate text-nowrap fs-6">{{ subIssue?.title }}</span>
    </div>
    <div v-if="!progress" class="subIssue-info d-flex align-items-center justify-content-between">
      <div class="position-relative">
        <div class="py-1 px-3 rounded-1 fw-bold fs-8 text-uppercase text-nowrap"
             :class="subIssue?.status?.name === 'TO DO' ? 'bg-light-active' : subIssue?.status?.name === 'DONE' ? 'bg-light-primary text-primary' : 'bg-light-success text-success'">
          <div v-if="EditStatusLoading">
            <Icon name="svg-spinners:180-ring-with-bg" size="20"/>
          </div>
          <div v-else v-click-out-side="closeStatuses" @click="statusesToggle">
        ...
        <Transition name="statusesMenu">
          <div v-if="statusesOpen"
            ...
            <div v-else v-for="(status, index) in backlogStore.statusesArray" :key="index"
              ...
              <div v-if="status?.name !== subIssue?.status?.name"
      ...
      <div class="position-relative">
        <div
          ...
          <div v-else v-click-out-side="closeAssignee" @click="assigneeToggle"
            ...
            <div v-if="subIssue?.assign_to">
              <img v-if="subIssue?.team_member?.user?.photo" :src="subIssue?.team_member?.user?.url_photo"
                   :alt="subIssue?.team_member?.user?.name" class="w-35px h-35px"/>
              <span v-else class="symbol-label text-inverse-warning fw-bold fs-3"
                    :style="{ backgroundColor: getColor(subIssue?.team_member?.user?.id) }">{{
                  subIssue?.team_member?.user?.name ? subIssue?.team_member?.user?.name[0].toUpperCase() : '-'
                }}</span>
            </div>
            <div v-else>
        ...
        <Transition name="statusesMenu">
          <div v-if="assigneeOpen"
            ...
            <div @click="changesubIssue({'assign_to': null})"
            ...
            <div
              ...
              <div class="w-100 d-flex justify-content-start align-items-center hover-bg-light p-2 border-bottom"
                ...
                <div class="symbol symbol-circle symbol-30px overflow-hidden">
                  ...
                  <span v-else class="symbol-label text-inverse-warning fs-3 fw-bold"
      ...
      <div class="position-relative">
        <div class=" py-1 px-3 rounded-1 fw-bold fs-8 text-uppercase text-nowrap">
          ...
          <div v-else v-click-out-side="closeSubIssueMenu" @click="subIssueMenuToggle"
    ...
    <icon v-else name="svg-spinners:ring-resize" class="text-gray-600 me-3 p-1" size="25"/>
  </div>
  <!--  <issueComponentIssueInfo />-->
</template>
