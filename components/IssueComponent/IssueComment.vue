...
const handleDeleteComment = () => {
  Swal.fire({
    ...
    preConfirm: async () => {
      await deleteComment(projectsStore?.project?.project_identify, props?.comment?.issue_id, props?.comment?.id);
...
const handleEditComment = async () => {
  ...
  if(commentValue.value !== props?.comment?.content) {
    try {
      await editComment(projectsStore?.project?.project_identify, props?.comment?.issue_id, props?.comment?.id, {'content': commentValue.value, mentionList: []});
      await backlogStore?.getIssueComments(projectsStore?.project?.project_identify, backlogStore?.issueInfoArray?.id);
    } catch (error) {
...
<template>
  <div>
    <div v-if="!editCommentStatus" class="comment-content row align-baseline py-3 my-4 rounded-1 overflow-hidden">
      <div class="d-flex justify-content-between">
          <div class="d-flex align-items-center overflow-hidden cursor-pointer">
            <img v-if="comment?.user?.photo"
                 ...
                 :alt="comment?.user?.name" class="w-35px h-35px rounded-circle"/>
            <span v-else class="symbol-label text-inverse-warning w-30px h-30px fs-1 d-flex justify-content-center align-items-center rounded-circle"
            ...
            <div class="d-flex flex-column ms-2">
              ...
              <span class="fs-8 text-gray-500">{{ moment(comment?.updated_at).fromNow() }}</span>
            </div>
          </div>
        <div class="position-relative">
          <div class=" py-1 px-3 rounded-1 fw-bold fs-8 text-uppercase text-nowrap">
            <div v-click-out-side="closeCommentMenu" @click="commentMenuToggle"
    ...
    <div v-else>
        <div class="d-flex align-items-center overflow-hidden cursor-pointer">
          <img v-if="comment.user.photo"
               :src="comment.user.url_photo"
               :alt="comment.user.name" class="w-35px h-35px rounded-circle"/>
          <span v-else class="symbol-label text-inverse-warning fs-2 d-flex justify-content-center align-items-center w-35px h-35px rounded-circle"
                :style="{ backgroundColor: getColor(comment.user.id) }">{{
              comment.user.name? comment.user.name[0].toUpperCase() : '-'
            }}</span>
          <div class="d-flex flex-column">
            <span class="fs-6 ms-3 fw-semibold">{{ comment.user.name }}</span>
            <span class="fs-8 text-gray-600 ms-3">{{ moment(comment.updated_at).fromNow() }}</span>
          </div>
        </div>
      <div>
        <div class="w-100 border-0 pt-3 ps-14 card ms-auto">
          <form @submit.prevent="handleEditComment">
            ...
            <div class="d-flex justify-content-end my-3">
              <button type="submit" :disabled="commentIssueLoader"
              ...
              <p @click="editCommentStatus = false" class="btn btn-sm btn-secondary mx-1">Cansel</p>
            </div>
          </form>
        </div>
      </div>
