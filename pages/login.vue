...
function formHandle() {
  !formData.email ? errorMsg.errorEmail = 'Email is required' : !formData.password ? errorMsg.errorPassword = 'Password is required' : '';
...
<template>
  <div>
    <div class="position-absolute w-100 h-100 d-flex justify-content-center align-items-center" id="kt_app_root">
      <div class="d-flex flex-column flex-lg-row-fluid w-lg-50 p-10 order-2 order-lg-1 m-auto">
          ...
          <div class="w-500px p-10 border rounded-1 text-center">
            ...
            <form class="form w-100" @submit.prevent="formHandle">
              ...
              <FormInput v-model:input="formData.email" type="text" placeholder="test@test.com" autocomplete="off"
                         labelText="Email" name="email" :formDataError="errorMsg.errorEmail" class="my-7"/>

              <FormPasswordInput v-model:input="formData.password" placeholder="password" autocomplete="off"
                                 labelText="Password" name="password" :formDataError="errorMsg.errorPassword"
                                 class="mt-7"/>
              <div class="d-flex flex-stack flex-wrap gap-3 fs-base fw-light mb-8">
                <!--begin::Link-->
                <div></div>
                <nuxt-link to="/forget-password" class="link-primary">Forgot Password ?</nuxt-link>
                <!--end::Link-->
              </div>
