...
onBeforeMount(async () => {
  try {
    ...
    const response = await welcome({identify_number: route.query.identify_number})
  ...
  emailImg.value = 'assets/media/email/email.svg'
})
...
<template>
  <div class="page-background">
    <div class="d-flex flex-column vh-100 mh-100 justify-content-center mx-auto w-500 text-center">
      <div v-if="!inProgress" class="d-flex flex-column mx-auto bg-white shadow box-sizing-border-box text-dark"
        ...
        <div class="d-flex flex-column align-items-center text-center custom-img">
          ...
          <img :src="emailImg" style="height: 88px; margin: 8px 0px 16px;">
        </div>
        ...
        <div class="resend-button mt-5">
          <button class="bg-transparent border-0 text-primary" @click="showToast('success', 'check Your email Box')">
            Resend verification email
          </button>
        </div>
        <div class="pt-4 border-top border-gray-600 mt-4">
          <p>You have only <b class="text-primary">{{ mixCountOfRequest - 2 }}</b> attempts left to resend
            the email.</p>
        </div>
      </div>
      <div v-else>
