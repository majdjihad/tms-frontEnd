...
function formHandle() {
  !data.name ? errorMsg.errorName = 'UserName is required' : !data.password ? errorMsg.errorPassword = 'Password is required' : !data.password_confirmation ? errorMsg.errorConfirmPassword = 'Confirm Password is required' : data.password !== data.confirmPassword ? errorMsg.errorConfirmPassword = 'Confirm password doesn\'t match the password' : '';
  ...
  if (data.password === data.password_confirmation && data.name && data.password) {
...
const { submit, inProgress, validationErrors: errors, } = useSubmit(() => {
  ...
  onSuccess: (response) => {
    ...
    if(invitationStore?.invitationInfo?.invite_identify) {
      return navigateTo(`/invitation?invite_identify=${invitationStore?.invitationInfo?.invite_identify}`, { replace: true })
        ...
        <div class="d-flex flex-column flex-lg-row-fluid w-lg-50 p-10 order-2 order-lg-1">
          <div class="d-flex flex-center flex-column flex-lg-row-fluid">
            <div class="w-500px p-10">
              <form class="form w-100 text-center" @submit.prevent="formHandle" id="completeForm">
                ...
                <FormInput type="text" autocomplete="off" labelText="Full Name" name="name" placeholder="Full Name"
                  v-model:input="data.name" :formDataError="errorMsg.errorName" class="mb-7" />

                <FormPasswordInput placeholder="Password" autocomplete="off" labelText="Password" name="password"
                  v-model:input="data.password" :formDataError="errorMsg.errorPassword" class="mb-7" />

                <FormPasswordInput placeholder="Confirm Password" autocomplete="off" labelText="Confirm Password"
                  name="password_confirmation" v-model:input="data.password_confirmation"
                  :formDataError="errorMsg.errorConfirmPassword" class="mb-7" />
                <!-- End Input Fields -->

                <div class="d-grid mb-10">
                  <button :disabled="inProgress" type="submit" id="kt_sign_in_submit" class="btn btn-primary btn-hover">
                    <span v-if="!inProgress" class="indicator-label">Complete registration</span>
        ...
        <div class="d-flex flex-lg-row-fluid w-lg-50 bgi-size-cover bgi-position-center order-1 order-lg-2 h-1"
          ...
          <div class="d-flex flex-column flex-center py-7 py-lg-15 px-5 px-md-15 w-100">
            ...
            <div class="d-none d-lg-block text-white fs-base text-center w-75">
              Doloremque in quam et at corrupti cupiditate quis quibusdam nemo,voluptates, voluptatum rem inventore? Nulla
