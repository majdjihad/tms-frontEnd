...
export const useBacklogStore = defineStore('useBacklog', () => {
    ...
    const { backlog, statuses, issueDescription, issueComments, issueHistory } = useBacklog();
    ...
    const getIssueHistory = async (projectId, issueId, pageNumber) => {
        if(issueHistoryArray.value !== []) {
            try {
                const issueHistoryResponse = await issueHistory(projectId, issueId, pageNumber)
